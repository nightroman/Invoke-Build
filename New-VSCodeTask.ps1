...
Add-Line '    "options": {'
Add-Line '      "shell": {'
Add-Line '        "executable": "{0}",' ($Shell.Replace('\', '\\'))
...
# get task list, optionally filtered
if ($WhereTask) {
	$tasks = @($all.Values | Where-Object $WhereTask)
}
else {
	$tasks = $all.Values
}
